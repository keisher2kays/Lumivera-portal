...
//           <img src={logoImg} alt="LumiVera Green Energy" className="admin-logo-img" style={{ height: '67px', width: 'auto'  }} />
      ...
      <main style={styles.contentArea}>
          ...
          <div style={styles.chatLayout} className="admin-chat-layout">
            <div style={styles.alertsPanel} className="admin-alerts-panel">
              ...
              {alerts.length === 0? (
                ...
                alerts.map((alert) => (
                  <div key={alert.roomId} style={styles.alertCard}>
                    <p style={{ margin: '0 0 8px 0', fontSize: '0.9rem' }}>⚠ Handover requested</p>
                    <button onClick={() => joinChatRoom(alert.roomId)} style={styles.joinBtn}>
                      Join Room: {alert.roomId}
                    </button>
                  </div>
            ...
            <div style={styles.chatWindow} className="admin-chat-window">
              <div style={styles.messageBox}>
                {messages.length === 0? (
                  ...
                  messages.map((m, idx) => (
                    <div key={idx} style={{...styles.msgRow, justifyContent: m.sender === 'agent'? 'flex-end' : 'flex-start' }}>
                      <div style={{...styles.msgBubble, background: m.sender === 'agent'? '#2CA636' : m.sender === 'user'? '#1E293B' : '#334155', color: '#FFFFFF' }}>
                        <span style={styles.senderLabel}>[{m.sender}]:</span> {m.text || m.message}
                      </div>
                    </div>
                  ))
              ...
              <form onSubmit={handleSendMessage} style={styles.inputForm}>
                <input type="text" placeholder={currentRoom? 'Type reply as LumiVera Agent...' : 'Join a chat room first'} value={agentInput} onChange={(e) => setAgentInput(e.target.value)} disabled={!currentRoom} style={styles.textInput} />
                <button type="submit" disabled={!currentRoom} style={styles.sendBtn}>Send</button>
              </form>
            </div>
          </div>
        )}
        ...
        {activeTab === 'products' && (
          <div>
            <div style={styles.tableCard} className="admin-table-card">
              <h3 style={{ marginTop: 0 }}>Add New Product</h3>
              {productError && <div style={{ background: '#7F1D1D', color: '#FECACA', padding: '12px', borderRadius: '6px', marginBottom: '15px' }}>{productError}</div>}
              {productSuccess && <div style={{ background: '#064E3B', color: '#A7F3D0', padding: '12px', borderRadius: '6px', marginBottom: '15px' }}>{productSuccess}</div>}
              <form onSubmit={handleCreateProduct}>
                <div style={styles.formGrid} className="admin-form-grid">
                  ...
                  <input type="number" placeholder="Price USD (Zimbabwe)" value={productForm.priceZimUSD} onChange={(e) => setProductForm({...productForm, priceZimUSD: e.target.value })} style={styles.input} required />
                  <input type="number" placeholder="Price GBP (UK) — optional" value={productForm.priceUKGBP} onChange={(e) => setProductForm({...productForm, priceUKGBP: e.target.value })} style={styles.input} />
                  <input type="number" min="0" placeholder="Quantity in Stock" value={productForm.stockQuantity} onChange={(e) => setProductForm({...productForm, stockQuantity: e.target.value })} style={styles.input} />
                </div>
                <textarea placeholder="Description" value={productForm.description} onChange={(e) => setProductForm({...productForm, description: e.target.value })} style={{...styles.input, width: '100%', marginTop: '16px', minHeight: '70px', boxSizing: 'border-box' }} />
                <input type="text" placeholder="Specs (comma-separated)" value={productForm.specs} onChange={(e) => setProductForm({...productForm, specs: e.target.value })} style={{...styles.input, width: '100%', marginTop: '16px', boxSizing: 'border-box' }} />
                <button type="submit" style={styles.submitBtn}>+ Add Product</button>
              </form>
            </div>
                    ...
                    {products.length === 0? (
                      ...
                      products.map((p) => (
                        <tr key={p._id}>
                          ...
                          <td>{p.category}</td>
                          <td>${p.priceZimUSD}</td>
                          <td><input type="number" min="0" defaultValue={p.stockQuantity?? 0} onBlur={(e) => handleStockChange(p._id, e.target.value)} style={{...styles.input, width: '70px', padding: '6px 8px' }} /></td>
                          <td>{(p.stockQuantity?? 0) > 0? '✅ In Stock' : '❌ Out of Stock'}</td>
                          <td style={{ textAlign: 'right' }}><button onClick={() => handleDeleteProduct(p._id)} style={styles.deleteBtn}>Delete</button></td>
                        </tr>
                      ))
                    )}
                  </tbody>
              ...
              <table style={styles.table}>
                <thead>
                  <tr>
                    ...
                    <th style={{ minWidth: '170px' }}>Installation Date</th>
                    <th style={{ minWidth: '240px' }}>Package</th>
                    <th style={{ minWidth: '130px' }}>Slots Filled</th>
                    ...
                    installations.flatMap((inst) =>
                      inst.clients.map((client) => (
                        <tr key={`${inst._id}-${client._id}`}>
                          ...
                          <td>{client.phone}</td>
                          <td>{new Date(inst.date).toLocaleDateString()}</td>
                          <td style={{ whiteSpace: 'normal', maxWidth: '300px' }}>{client.packageName || 'Not specified'}</td>
                          <td>{inst.bookedSlots} / {inst.maxSlots}</td>
                          <td style={{ textAlign: 'right' }}>
                            <button onClick={() => handleDeleteInstallationClient(inst._id, client._id)} style={styles.deleteBtn}>Remove</button>
                          </td>
                        </tr>
                      ))
                    )
...
const styles = {
  ...
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '20px', borderBottom: '1px solid #1E293B' },
  statusBadge: { background: '#1E293B', padding: '6px 12px', borderRadius: '20px', fontSize: '0.85rem', color: '#38BDF8' },
  navTabs: { display: 'flex', gap: '12px', margin: '20px 0', flexWrap: 'wrap' },
  tabBtn: { background: '#1E293B', color: '#94A3B8', border: 'none', padding: '10px 18px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' },
  activeTabBtn: { background: '#2CA636', color: '#FFFFFF', border: 'none', padding: '10px 18px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' },
  contentArea: { marginTop: '20px' },
  chatLayout: { display: 'grid', gridTemplateColumns: '320px 1fr', gap: '20px' },
  ...
  textInput: { flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid #CBD5E1', fontSize: '0.95rem' },
  sendBtn: { background: '#2CA636', color: '#FFFFFF', border: 'none', padding: '0 24px', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' },
  tableCard: { background: '#1E293B', borderRadius: '12px', padding: '24px', marginBottom: '24px', overflow: 'hidden' },
  table: { width: '100%', borderCollapse: 'collapse', marginTop: '16px', textAlign: 'left' },
  formGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' },
  input: { padding: '12px', borderRadius: '8px', border: '1px solid #334155', background: '#0F172A', color: '#F8FAFC', fontSize: '0.95rem' },
  submitBtn: { marginTop: '16px', background: '#2CA636', color: '#FFFFFF', border: 'none', padding: '12px 24px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' },
